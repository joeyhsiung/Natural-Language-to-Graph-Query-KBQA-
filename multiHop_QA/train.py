...
class LoadData:
    ...
    def __split_data(self):
        ...
        train_q = self.read_docs_to_seqs(train['question_sep'].values, self.q2i)
        train_rel = self.read_docs_to_seqs(train['relationship'].values, self.rel2id,is_rel = True)
        val_q = self.read_docs_to_seqs(test['question_sep'][0:split].values, self.q2i)
        val_rel = self.read_docs_to_seqs(test['relationship'][0:split].values, self.rel2id,is_rel = True)
        test_q = self.read_docs_to_seqs(test['question_sep'][split:-1].values, self.q2i)
...
if __name__ == "__main__":
    ...
    model_name = "0924_transformers_q_sep_best-model-1e4lr.pt"
    INPUT_DIM = len(q2id) ##
    print('INPUT_DIM',INPUT_DIM)
    OUTPUT_DIM = len(rel2id) ## index problem should be here
    ...
    for epoch in range(hper_params.N_EPOCHS):
        ...
        train_iterator = iterate_batches(pairs=train_pairs, batch_size=hper_params.BATCH_SIZE, shuffle=True)
